...
public class TestSetupRepository : ITestSetupRepository
{
    ...
    public async Task DeleteAsync(int id, CancellationToken cancellationToken = default)
    {
        try
        {
            var testSetup = await GetByIdAsync(id, cancellationToken);
            if (testSetup != null)
            {
                testSetup.IsActive = false;
                testSetup.ModifiedDate = DateTime.UtcNow;
                await UpdateAsync(testSetup, cancellationToken);
            }
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> ExistsAsync(string name, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(ex, "Error checking if test setup exists: {Name}", name);
            throw;
        }
    }
