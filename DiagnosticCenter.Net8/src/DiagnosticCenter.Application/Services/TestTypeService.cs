...
public class TestTypeService : ITestTypeService
{
    ...
    public async Task<TestType> CreateAsync(TestType testType, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (await _repository.NameExistsAsync(testType.Name, cancellationToken))
            {
                throw new DuplicateEntityException("TestType", "Name", testType.Name);
            }
            ...
            _logger.LogInformation("Successfully created test type with ID: {Id}", created.Id);
            return created;
        }
        catch (Exception ex)
        {
    ...
    public async Task UpdateAsync(int id, TestType testType, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (existing == null)
            {
                throw new EntityNotFoundException("TestType", id);
            }
    ...
    public async Task<bool> NameExistsAsync(string name, CancellationToken cancellationToken = default)
    {
        try
        {
            return await _repository.NameExistsAsync(name, cancellationToken);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error checking if test type name exists: {Name}", name);
            throw;
        }
    }
